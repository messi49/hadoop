...
public class TestInMemoryPlan {
  ...
  public void setUp() throws PlanningException {
    ...
    maxAlloc = Resource.newInstance(64 * 1024, 20, 8 * 256);
    totalCapacity = Resource.newInstance(100 * 1024, 100, 16 * 256);
  ...
  public void testAddReservation() {
    ...
    for (int i = 0; i < alloc.length; i++) {
      Assert.assertEquals(Resource.newInstance(1024 * (alloc[i]), (alloc[i]), 256 * (alloc[i])),
    ...
    for (int i = 0; i < updatedAlloc.length; i++) {
      Assert.assertEquals(
          Resource.newInstance(1024 * (updatedAlloc[i] + i), updatedAlloc[i]
              + i, 256 * (updatedAlloc[i] + i)), plan.getTotalCommittedResources(start + i));
      Assert.assertEquals(
          Resource.newInstance(1024 * (updatedAlloc[i] + i), updatedAlloc[i]
              + i, 256 * (updatedAlloc[i] + i)), plan.getConsumptionForUser(user, start + i));
    }
  }
  ...
  public void testDeleteReservation() {
    ...
    for (int i = 0; i < alloc.length; i++) {
      Assert.assertEquals(
          Resource.newInstance(1024 * (alloc[i] + i), (alloc[i] + i), 256 * (alloc[i] + i)),
          plan.getTotalCommittedResources(start + i));
      Assert.assertEquals(
          Resource.newInstance(1024 * (alloc[i] + i), (alloc[i] + i), 256 * (alloc[i] + i)),
          plan.getConsumptionForUser(user, start + i));
  ...
  public void testArchiveCompletedReservations() {
    ...
    for (int i = 0; i < alloc1.length; i++) {
      Assert.assertEquals(
          Resource.newInstance(1024 * (alloc1[i]), (alloc1[i]), 256 * (alloc1[i])),
          plan.getTotalCommittedResources(start + i));
      Assert.assertEquals(
          Resource.newInstance(1024 * (alloc1[i]), (alloc1[i]), 256 * (alloc1[i])),
          plan.getConsumptionForUser(user, start + i));
    ...
    for (int i = 0; i < alloc2.length; i++) {
      Assert.assertEquals(
          Resource.newInstance(1024 * (alloc1[i] + alloc2[i] + i), alloc1[i]
              + alloc2[i] + i, 256 * (alloc1[i] + alloc2[i] + i)), plan.getTotalCommittedResources(start + i));
      Assert.assertEquals(
          Resource.newInstance(1024 * (alloc1[i] + alloc2[i] + i), alloc1[i]
              + alloc2[i] + i, 256 * (alloc1[i] + alloc2[i] + i)), plan.getConsumptionForUser(user, start + i));
    }
