...
public class TestQueueMetrics {
  ...
  @Test public void testDefaultSingleQueueMetrics() {
    ...
    metrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 1536 * 4));
    metrics.incrPendingResources(user, 5, Resources.createResource(3*GB, 3, 256));
    ...
    metrics.allocateResources(user, 3, Resources.createResource(2*GB, 2, 256), true);
    checkResources(queueSource, 6*GB, 6, 3, 3, 0, 100*GB, 100, 9*GB, 9, 2, 0, 0, 0);

    metrics.releaseResources(user, 1, Resources.createResource(2*GB, 2, 256));
  ...
  @Test public void testSingleQueueWithUserMetrics() {
    ...
    metrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 1536 * 4));
    metrics.setAvailableResourcesToUser(user, Resources.createResource(10*GB, 10, 256));
    metrics.incrPendingResources(user, 5, Resources.createResource(3*GB, 3, 256));
    // Available resources is set externally, as it depends on dynamic
    // configurable cluster/queue resources
    checkResources(queueSource, 0, 0, 0, 0, 0,  100*GB, 100, 15*GB, 15, 5, 0, 0, 0);
  ...
  @Test public void testTwoLevelWithUserMetrics() {
    ...
    parentMetrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 1536 * 4));
    metrics.setAvailableResourcesToQueue(Resources.createResource(100*GB, 100, 1536 * 4));
    parentMetrics.setAvailableResourcesToUser(user, Resources.createResource(10*GB, 10, 256));
    metrics.setAvailableResourcesToUser(user, Resources.createResource(10*GB, 10, 256));
    metrics.incrPendingResources(user, 5, Resources.createResource(3*GB, 3, 256));
    checkResources(queueSource, 0, 0, 0, 0, 0, 100*GB, 100, 15*GB, 15, 5, 0, 0, 0);
    ...
    metrics.allocateResources(user, 3, Resources.createResource(2*GB, 2, 256), true);
    metrics.reserveResource(user, Resources.createResource(3*GB, 3, 256));
    // Available resources is set externally, as it depends on dynamic
    // configurable cluster/queue resources
    ...
    metrics.releaseResources(user, 1, Resources.createResource(2*GB, 2, 256));
    metrics.unreserveResource(user, Resources.createResource(3*GB, 3, 256));
    checkResources(queueSource, 4*GB, 4, 2, 3, 1, 100*GB, 100, 9*GB, 9, 2, 0, 0, 0);
